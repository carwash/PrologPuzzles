...
/* count_list_all(N,L,M). k list has N = number is N^K
	...
	Many = 16
*/
count_list_all(N,L,Many) :-
	findall(X,all(N,L,X),Res),
	length(Res,Many).
...
*/
neighbor(X,Y,X1,Y1,S) :-
	((X1 is X+1, Y=Y1); (X1 is X-1, Y=Y1);
	 (X1=X, Y1 is Y+1); (X1=X, Y1 is Y-1);
	 (X1 is X+1, Y1 is Y-1);
	 (X1 is X-1, Y1 is Y-1);
	 (X1 is X+1, Y1 is Y+1);
	 (X1 is X-1, Y1 is Y+1)),
...
neighbour(X,Y,X1,Y1,S) :- neighbor(X,Y,X1,Y1,S).
